...
bool f(string &pre, string &s){
	if(pre.size() > s.size())return false; //no son prefijo
	return s.substr(0,pre.size())==pre; //son o no son prefijo
}

struct PrefixFreeSets {
	int maxElements(vector <string> v) {
		int res = 0;
		make_unique(v);
