...
double f(int candies){
	if(candies <= 0)return 0;
	double &best = memo[candies];
	if(mark[candies])return best;
	mark[candies] = true;
	best = 0;
	for(int i=1; i<=6; ++i)
		best += (f(candies-i)+1.0)*p;
	return best;
}
...
namespace moj_harness {
	...
	int run_test_case( int casenum ) {
		...
		case 0: {
			...
			return verify_case( casenum, expected, TheDiceGame().expectedThrows( candies ) );
		}
		case 1: {
			int candies               = 2;
